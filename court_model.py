...
def is_already_reported(item_type, item_id):
    conn = psycopg2.connect(os.getenv('DATABASE_URL'))
    c = conn.cursor()
    c.execute('SELECT id FROM court_entries WHERE item_type=%s AND item_id=%s', (item_type, item_id))
    result = c.fetchone()
    conn.close()
...
def add_court_entry(item_type, item_id, category, content, posted_by, reported_by):
    ...
    c = conn.cursor()
    c.execute(
        'INSERT INTO court_entries (item_type, item_id, category, content, posted_by, reported_by) VALUES (%s, %s, %s, %s, %s, %s)',
...
def get_court_entries(category):
    conn = psycopg2.connect(os.getenv('DATABASE_URL'))
    c = conn.cursor()
    c.execute('SELECT * FROM court_entries WHERE category=%s ORDER BY created_at DESC', (category,))
    entries = c.fetchall()
    conn.close()
    return entries
...
def add_vote(entry_id, nickname, vote_type): 
    ...
    c.execute('SELECT 1 FROM court_delete_votes WHERE entry_id=%s AND nickname=%s', (entry_id, nickname))
    if c.fetchone():
        conn.close()
        return False

    c.execute('INSERT INTO court_delete_votes (entry_id, nickname, vote_type) VALUES (%s, %s, %s)', (entry_id, nickname, vote_type))

    column = 'delete_votes' if vote_type == 'delete' else 'keep_votes'
    c.execute(f'UPDATE court_entries SET {column} = {column} + 1 WHERE id=%s', (entry_id,))
...
def get_court_reasons(entry_id, nickname=None): 
    conn = psycopg2.connect(os.getenv('DATABASE_URL'))
    c = conn.cursor()
    c.execute('SELECT reason1, reason1_by, reason1_upvotes, reason2, reason2_by, reason2_upvotes, reason3, reason3_by, reason3_upvotes FROM court_reasons WHERE entry_id=%s', (entry_id,))
    row = c.fetchone()
    conn.close()
...
def fill_court_reason_slot(entry_id, slot_num, text, submitted_by): #
    ...
    c.execute('SELECT reason1, reason2, reason3 FROM court_reasons WHERE entry_id=%s', (entry_id,))
    row = c.fetchone()

    if row is None:
        c.execute('INSERT INTO court_reasons (entry_id) VALUES (%s)', (entry_id,))
        row = (None, None, None)
    ...
    c.execute(f'UPDATE court_reasons SET {col}=%s, {by_col}=%s WHERE entry_id=%s', (text, submitted_by, entry_id))
    conn.commit()
    conn.close()
    return True
...
def get_court_entries_with_vote(category): #
    ...
    c.execute('''
        ...
        FROM court_entries WHERE category=%s ORDER BY created_at DESC
    ''', (category,))
    entries = c.fetchall()
    conn.close()
    return entries
...
def set_delete_vote_threshold(value):
    conn = psycopg2.connect(os.getenv('DATABASE_URL'))
    c = conn.cursor()
    c.execute("UPDATE court_settings SET value=%s WHERE key='delete_vote_threshold'", (str(value),))
    conn.commit()
    conn.close()
...
def get_notifications(nickname):
    conn = psycopg2.connect(os.getenv('DATABASE_URL'))
    c = conn.cursor()
    c.execute('SELECT id message, created_at, is_read FROM notifications WHERE nickname=%s AND is_read=0 ORDER BY created_at DESC', (nickname,))
    rows = c.fetchall()
    conn.close()
    return rows
...
def auto_delete_if_threshold_met(entry_id):
    conn = psycopg2.connect(os.getenv('DATABASE_URL'))
    c = conn.cursor()
    c.execute('SELECT item_type, item_id, posted_by, delete_votes FROM court_entries WHERE id=%s', (entry_id,))
    row = c.fetchone()
    conn.close()
...
def mark_notifications_read(nickname):
    ...
    c.execute('UPDATE notifications SET is_read=1, read_at=%s WHERE nickname=%s AND is_read=0', (now, nickname,))
    conn.commit()
    conn.close()
...
def get_setting(key, default):
    conn = psycopg2.connect(os.getenv('DATABASE_URL'))
    c = conn.cursor()
    c.execute('SELECT value FROM court_settings WHERE key=%s', (key,))
    row = c.fetchone()
    conn.close()
...
def set_setting(key, value):
    conn = psycopg2.connect(os.getenv('DATABASE_URL'))
    c = conn.cursor()
    c.execute('INSERT OR REPLACE INTO court_settings (key, value) VALUES (%s, %s)', (key, str(value)))
    conn.commit()
    conn.close()
...
def get_all_notifications(nickname):
    ...
    c.execute("""
        ...
        WHERE nickname=%s AND is_read=1
        AND COALESCE(read_at, created_at) <= %s
    """, (nickname, cutoff_str))
    conn.commit()

    c.execute('SELECT id, message, created_at, is_read FROM notifications WHERE nickname=%s ORDER BY created_at DESC', (nickname,))
    rows = c.fetchall()
    conn.close()
    return rows
...
def has_user_submitted_reason(entry_id, nickname):
    conn = psycopg2.connect(os.getenv('DATABASE_URL'))
    c = conn.cursor()
    c.execute('SELECT reason1_by, reason2_by, reason3_by FROM court_reasons WHERE entry_id=%s', (entry_id,))
    row = c.fetchone()
    conn.close()
    if not row:
        return False
...
def has_user_upvoted_reason(entry_id, slot_num, nickname):
    conn = psycopg2.connect(os.getenv('DATABASE_URL'))
    c = conn.cursor()
    c.execute('SELECT 1 FROM court_reason_upvotes WHERE entry_id=%s AND slot_num=%s AND nickname=%s', (entry_id, slot_num, nickname))
    row = c.fetchone()
    conn.close()
    return row is not None
...
def add_reason_upvote(entry_id, slot_num, nickname):
    ...
    c.execute('INSERT INTO court_reason_upvotes (entry_id, slot_num, nickname) VALUES (%s, %s, %s)', (entry_id, slot_num, nickname))

    col = f'reason{slot_num}_upvotes'
    c.execute(f'UPDATE court_reasons SET {col} = {col} + 1 WHERE entry_id=%s', (entry_id,))
...
def get_justification(entry_id, nickname):
    conn = psycopg2.connect(os.getenv('DATABASE_URL'))
    c = conn.cursor()
    c.execute('SELECT posted_by, justification FROM court_entries WHERE id=%s', (entry_id,))
    row = c.fetchone()
    conn.close()
    if not row:
...
def set_justification(entry_id, nickname, text):
    conn = psycopg2.connect(os.getenv('DATABASE_URL'))
    c = conn.cursor()
    c.execute('SELECT posted_by FROM court_entries WHERE id=%s', (entry_id,))
    row = c.fetchone()
    if not row or row[0] != nickname:
        conn.close()
        return False
    c.execute('UPDATE court_entries SET justification=%s WHERE id=%s', (text, entry_id))
    conn.commit()
    conn.close()
    return True
