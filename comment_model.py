...
def add_comment(issue_id, topic_id, nickname, comment):
    ...
    c = conn.cursor()
    c.execute(
        'INSERT INTO comments (issue_id, topic_id, nickname, comment) VALUES (%s, %s, %s, %s)',
...
def update_comment(comment_id, nickname, new_comment):
    ...
    conn = psycopg2.connect(os.getenv('DATABASE_URL'))
    c = conn.cursor()
    c.execute('SELECT id FROM comments WHERE id = %s AND nickname = %s', (comment_id, nickname))
    owns = c.fetchone()
    if owns:
        c.execute('UPDATE comments SET comment = %s, edited = 1 WHERE id = %s AND nickname = %s',
...
def delete_comment(comment_id, nickname):
    ...
    if owns_comment:
        c.execute('DELETE FROM replies WHERE comment_id = %s', (comment_id,))
        c.execute('DELETE FROM comments WHERE id = %s AND nickname = %s', (comment_id, nickname))
        conn.commit()
...
def toggle_vote(comment_id, nickname, vote):
    conn = psycopg2.connect(os.getenv('DATABASE_URL'))
    c = conn.cursor()
    c.execute('SELECT vote FROM votes WHERE comment_id = %s AND nickname = %s', (comment_id, nickname))
    existing = c.fetchone()
    if existing:
        if int(existing[0]) == vote:
            c.execute('DELETE FROM votes WHERE comment_id = %s AND nickname = %s', (comment_id, nickname))
            new_user_vote = 0
            action = 'removed'
        else:
            c.execute('UPDATE votes SET vote = %s WHERE comment_id = %s AND nickname = %s', (vote, comment_id, nickname))
            new_user_vote = vote
            action = 'changed'
    else:
        c.execute('INSERT INTO votes (comment_id, nickname, vote) VALUES (%s, %s, %s)', (comment_id, nickname, vote))
        new_user_vote = vote
        action = 'added'
...
def get_votes(comment_id):
    conn = psycopg2.connect(os.getenv('DATABASE_URL'))
    c = conn.cursor()
    c.execute('SELECT vote FROM votes WHERE comment_id = %s', (comment_id,))
    votes = c.fetchall()
    conn.close()
...
def get_comment_retain_count(comment_id):
    conn = psycopg2.connect(os.getenv('DATABASE_URL'))
    c = conn.cursor()
    c.execute('SELECT retain_count FROM comments WHERE id=%s', (comment_id,))
    row = c.fetchone()
    conn.close()
...
def toggle_conclusion_vote(comment_id, nickname):
    conn = psycopg2.connect(os.getenv('DATABASE_URL'))
    c = conn.cursor()
    c.execute('SELECT 1 FROM conclusion_votes WHERE comment_id=%s AND nickname=%s', (comment_id, nickname))
    existing = c.fetchone()

    if existing:
        c.execute('DELETE FROM conclusion_votes WHERE comment_id=%s AND nickname=%s', (comment_id, nickname))
        new_state = False
    else:
        c.execute('INSERT INTO conclusion_votes (comment_id, nickname) VALUES (%s, %s)', (comment_id, nickname))
        new_state = True
...
def get_conclusion_count(comment_id):
    conn = psycopg2.connect(os.getenv('DATABASE_URL'))
    c = conn.cursor()
    c.execute('SELECT COUNT(*) FROM conclusion_votes WHERE comment_id=%s', (comment_id,))
    count = c.fetchone()[0]
    conn.close()
    return count
...
def toggle_replies(comment_id, nickname):
    conn = psycopg2.connect(os.getenv('DATABASE_URL'))
    c = conn.cursor()
    c.execute('SELECT id, replies_enabled FROM comments WHERE id = %s AND nickname = %s', (comment_id, nickname))
    row = c.fetchone()
    if not row:
        conn.close()
        return None
    ...
    c.execute('UPDATE comments SET replies_enabled = %s WHERE id = %s', (new_state, comment_id))

    if new_state == 0:
        c.execute('SELECT id FROM replies WHERE comment_id = %s', (comment_id,))
...
def get_comment_owner(comment_id):
    conn = psycopg2.connect(os.getenv('DATABASE_URL'))
    c = conn.cursor()
    c.execute('SELECT nickname FROM comments WHERE id=%s', (comment_id,))
    row = c.fetchone()
    conn.close()
